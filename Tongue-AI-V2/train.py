# train.py — Final clean version (CB-BCE + WeightedRandomSampler + AMP + per-class thresholds)
...
def evaluate_model_and_find_thresholds(model, data_loader, device, label_cols, threshold_grid=None):
    """
    回傳：
      - metrics_at_best: 以每類最佳閾值計算的 macro/micro F1 + per-class F1/Jaccard
      - metrics_at_05: 以 0.5 閾值計算的指標（方便對照）
      - best_thresholds: 每個類別各自的最佳 threshold
    """
    ...
    # 使用最佳 threshold 的整體分數（以 per-class F1 平均）
...
def train_model(train_csv, val_csv, image_root, label_cols,
                ...
                beta_cb=0.9999, sampler_power=1.0):
    clear_gpu_memory()
    ...
    # Model
    model = SignOrientedNetwork(num_classes=len(label_cols),
                                backbone='swin_base_patch4_window7_224',  # 或 '50resnet'
                                feature_dim=512)
    model.to(device)

    # Class-Balanced BCE Loss（依據訓練集統計的有效樣本數）
    criterion = ClassBalancedBCELoss(n_pos=n_pos_t.to(device),
